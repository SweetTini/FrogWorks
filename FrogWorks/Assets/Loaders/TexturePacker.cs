        ...
        private static TextureAtlas FromStream(string filePath)
        {
            ...
            if (stream != null)
            {
                using (stream)
                {
                    ...
                    var rootTexturePath = Path.Combine(
                        directory, root.AttrToString("imagePath"));
                    var rootTexture = Texture.Load(rootTexturePath);

                    foreach (XmlElement spriteNode in root.ChildNodes)
                    {
                        var key = Path.ChangeExtension(
                            spriteNode.AttrToString("n"), null);
                        var texture = rootTexture.ClipRegion(
                            spriteNode.AttrToRectangle("x", "y", "w", "h"));
